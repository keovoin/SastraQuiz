...
class _FunAndLearnTitleScreen extends State<FunAndLearnTitleScreen> {
  ...
  /// Calculates the number of questions to display for a comprehension.
  /// If fix question is enabled, returns the minimum between comprehension
  /// total questions and the system configured limit.
  /// Otherwise, returns the comprehension's total question count.
  String _getQuestionsCount(String comprehensionTotalQuestions) {
    final systemConfig = context.read<SystemConfigCubit>().systemConfigModel;
    final funAndLearnQuizTotalQuestion =
        systemConfig?.funNLearnQuizTotalQuestion;
    final funAndLearnQuizFixQuestion =
        systemConfig?.funNLearnQuizFixQuestion == 1;

    if (funAndLearnQuizFixQuestion) {
      final comprehensionCount = int.parse(comprehensionTotalQuestions);
      if (comprehensionCount < funAndLearnQuizTotalQuestion!) {
        return comprehensionTotalQuestions;
      }
      return funAndLearnQuizTotalQuestion.toString();
    }
    ...
    return BlocConsumer<ComprehensionCubit, ComprehensionState>(
      ...
      builder: (context, state) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (_, index) {
            final questionsCount = _getQuestionsCount(
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      '${context.tr('questionLbl')}: $questionsCount',
                                      style: TextStyle(
                                        fontSize: 14,
                                        color: colorScheme.onTertiary
