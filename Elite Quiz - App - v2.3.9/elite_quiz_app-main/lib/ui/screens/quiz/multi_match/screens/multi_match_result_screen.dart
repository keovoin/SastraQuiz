...
class _MultiMatchResultScreenState extends State<MultiMatchResultScreen> {
  ...
  late final int _currLevel = int.parse(
    widget.args.level ?? widget.args.questions.first.level,
  );
  ...
  Widget _buildResultButtons(BuildContext context) {
    ...
    return Column(
      children: [
        if (_currLevel == 0)
          ...[]
        else if (_isWinner && _currLevel != widget.args.totalLevels) ...[
  ...
  Widget _buildPlayAgainButton() {
    //TODO : Check this flow...play again should start from same level
