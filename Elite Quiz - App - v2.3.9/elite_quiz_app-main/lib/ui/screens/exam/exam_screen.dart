...
import 'package:flutterquiz/utils/screen_recording_detector.dart';
...
class _ExamScreenState extends State<ExamScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // Prevent screenshots and screen recording during exam
    ScreenProtector.preventScreenshotOn();

    // Start iOS screen recording detection
    if (Platform.isIOS) {
      _startScreenRecordingListener();
    }
  ...
  /// Start listening for iOS screen recording changes (event-based)
  void _startScreenRecordingListener() {
    try {
      _screenRecordingSubscription = ScreenRecordingDetector
          .onRecordingStateChanged
          .listen((isRecording) {
            if (mounted && isRecording != isScreenRecording) {
              setState(() => isScreenRecording = isRecording);
            }
          });
      ScreenRecordingDetector.startListening();
  ...
  void _stopScreenRecordingListener() {
    try {
      _screenRecordingSubscription?.cancel();
      _screenRecordingSubscription = null;
      ScreenRecordingDetector.stopListening();
    } on PlatformException catch (_) {
    } catch (_) {}
  }
  ...
  void navigateToResultScreen() {
    ...
    // Re-enable screenshots before leaving exam screen
    ScreenProtector.preventScreenshotOff();
  ...
  Widget _buildScreenRecordingOverlay() {
    return Container(
      ...
      child: Center(
        child: Text(
          context.tr(screenRecordingNotAllowedKey) ??
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            ...
            // Show black overlay when screen recording is detected (iOS)
            if (isScreenRecording) _buildScreenRecordingOverlay(),
          ],
        ),
      ),
    );
