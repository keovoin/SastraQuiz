...
class BannerAdCubit extends Cubit<BannerAdState> {
  ...
  void initBannerAd(BuildContext context, {bool forceReload = false}) {
    ...
    // Don't reinitialize if ad is already loaded or loading (unless forced)
    if (!forceReload &&
        (state == BannerAdState.loaded || state == BannerAdState.loading)) {
      return;
    }

    emit(BannerAdState.loading);
